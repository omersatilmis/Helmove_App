...
class _BikeCardWidgetState extends State<BikeCardWidget> {
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                // FAVORİ BUTONU
                IconButton(
                  onPressed: () => setState(() => widget.bike.isFavorite = !widget.bike.isFavorite),
                  icon: Icon(
                    widget.bike.isFavorite ? Icons.favorite_rounded : Icons.favorite_border_rounded,
                    color: widget.bike.isFavorite ? Colors.redAccent : theme.colorScheme.onSurface.withValues(alpha: 0.3),
                  ),
                ),
                // AKSİYON BUTONLARI
                _isEditing ? _buildEditActions(theme) : _buildViewActions(theme),
              ],
            ),
          ),
  ...
  Widget _buildViewInfo(ThemeData theme) {
    final labelStyle = AppTextStyles.bodySmall.copyWith(color: theme.colorScheme.onSurface.withValues(alpha: 0.5), fontSize: 12);
    final valueStyle = AppTextStyles.medium.copyWith(color: theme.colorScheme.onSurface, fontSize: 15);

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _infoRow("Marka ve Model", widget.bike.makeModel, labelStyle, valueStyle.copyWith(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 12),
        Row(
          children: [
            Expanded(child: _infoRow("CC", widget.bike.cc, labelStyle, valueStyle)),
            Expanded(child: _infoRow("Yıl", widget.bike.year, labelStyle, valueStyle)),
          ],
        ),
        const SizedBox(height: 12),
        Row(
          children: [
            Expanded(child: _infoRow("Renk", widget.bike.color, labelStyle, valueStyle)),
            Expanded(child: _infoRow("Plaka", widget.bike.plate, labelStyle, valueStyle)),
          ],
        ),
        if (widget.bike.description.isNotEmpty) ...[
          ...
          Text(widget.bike.description, style: AppTextStyles.thin.copyWith(fontSize: 14)),
        ]
      ],
    );
  }
      ...
      children: [
        ...
        Row(
          children: [
            ...
            Expanded(child: _editField(theme, "Yıl", _yearCtrl, isNumber: true)),
          ],
        ),
        const SizedBox(height: 10),
        Row(
  ...
  Widget _editField(ThemeData theme, String label, TextEditingController ctrl, {bool isNumber = false, int maxLines = 1}) {
    return TextField(
      controller: ctrl,
      maxLines: maxLines,
